...
router.get('/search',isLoggedIn, async (req, res) => {
    ...
    const movies=await  fetch(`http://www.omdbapi.com/?s=${query}&page=1&apikey=${apikey}`)
    .then((res)=>res.json())
    .then((data)=>{
      return data
    })
...
router.post("/movie/:id",isLoggedIn,async(req,res)=>{
  const {id}=req.params;
  const movie=await fetch(`http://www.omdbapi.com/?i=${id}&page=2&apikey=${apikey}`)
  .then((res)=>res.json())
  .then((data)=>{
    return data
  })
...
router.get("/:name/:id/details",async(req,res)=>{
    const {id}=req.params;
    const movie=await fetch(`http://www.omdbapi.com/?i=${id}&page=1&apikey=${apikey}`)
    .then((res)=>res.json())
    .then((data)=>{
      return data
    })
...
router.get("/:movie/playlist",isLoggedIn,async (req,res)=>{
...
router.delete("/:uid/:type/:id",isLoggedIn,async(req,res)=>{
